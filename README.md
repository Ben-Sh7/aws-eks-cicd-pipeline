...
# the only secret the AWS deployment needs from you
TF_VAR_github_pat=<your GitHub PAT>
```
Grafana's and Jenkins' admin passwords are generated by Terraform into Secrets Manager, and the database password is generated and rotated by AWS - you never pick or handle any of them. `terraform output` prints the commands to read them.
